...
export class MotoCalendarPageComponent {
  ...
  raceCardLink(card: MotoCalendarCard): (string | number)[] {
    return [`/${this.motoCtx.id()}`, 'calendario', card.slug, card.defaultSession];
  }
  ...
  private fetchCalendar(): void {
    ...
    this.error.set(null);
    this.motoLive.getCalendar().pipe(
      tap(calendar => {
  ...
  private loadCompletedResults(calendar: JolpikaCalendarRace[]): void {
    ...
    forkJoin(
      completed.map(race =>
        this.motoLive.getRaceResults(race.round).pipe(
