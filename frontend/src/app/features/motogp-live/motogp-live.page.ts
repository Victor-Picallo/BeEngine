...
export class MotogpLivePageComponent implements OnInit, OnDestroy {
  ...
  private readonly service = inject(MotogpPulseService);
  ...
  seriesLabel = computed(() => this.seriesCtx.config().short);

  backLabel = computed(() =>
    this.backNav.labelFor(this.returnUrl(), `${this.seriesCtx.homePath()}/calendario`),
  );
    ...
    effect(() => {
      ...
      untracked(() => {
        void this.router.navigate(
          [this.seriesCtx.homePath(), 'calendario', slug, fallback],
          { replaceUrl: true },
        );
      });
    });
  ...
  goBack(): void {
    this.backNav.goBack(`${this.seriesCtx.homePath()}/calendario`, this.returnUrl());
  }
  ...
  buildSessionLink(key: string): (string | number)[] {
    ...
    return [this.seriesCtx.homePath(), 'calendario', slug, key];
  }

  driverLink(driverId: string | null | undefined): (string | number)[] | null {
    const id = (driverId ?? '').trim();
    if (!id || id === 'unknown') return null;
    return [this.seriesCtx.homePath(), 'pilotos', id];
  }
  ...
  private bootstrap(): void {
    ...
    this.service.getCalendar().subscribe({
      next: (cal) => {
        ...
        if (!slug) {
          void this.router.navigate(
            [this.seriesCtx.homePath(), 'calendario', slugifyRace(race), this.activeSessionKey()],
            { replaceUrl: true },
          );
        }
