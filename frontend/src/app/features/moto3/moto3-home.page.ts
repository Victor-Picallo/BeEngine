...
export class Moto3HomePageComponent implements OnInit, OnDestroy {
  ...
  private loadAll(): void {
    ...
    startHybridLoad(
      [
        {
          load: () => this.moto3.getCalendar().pipe(catchError(() => of([] as JolpikaCalendarRace[]))),
          onValue: (calendar) => this.calendarRaces.set(calendar as JolpikaCalendarRace[]),
        },
        {
          load: () =>
            this.moto3.getDriverStandingsResponse().pipe(
        ...
        {
          load: () =>
            this.moto3.getOfficialTeamsGrid().pipe(catchError(() => of([] as Moto3TeamStanding[]))),
        ...
        {
          load: () => this.moto3.getNextRace().pipe(catchError(() => of(null))),
          onValue: (nr) => this.nextRaceRaw.set(nr as Moto3NextRacePayload | null),
        },
        {
          load: () =>
            this.newsService.getFeed('moto3', 'Todos', 6, 0).pipe(
              catchError(() =>
                of({
                  items: [],
                  total: 0,
  ...
  private refreshAll(): void {
    ...
    this.moto3
      ...
      .pipe(catchError(() => of([] as Moto3TeamStanding[])), takeUntilDestroyed(this.destroyRef))
