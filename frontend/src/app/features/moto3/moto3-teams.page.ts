...
import { moto3TeamLogoGridClass, moto3TeamLogoUrl } from './moto3-media';
...
export interface Moto3TeamCard {
  pos: number;
  team: string;
...
function buildCards(rows: Moto3TeamStanding[]): Moto3TeamCard[] {
  return rows.map((r) => {
    ...
    return {
      ...
      imageUrl: moto3TeamLogoUrl(r.constructorId, r.teamId, r.logoUrl, r.team),
    };
  });
}

@Component({
  selector: 'app-moto3-teams-page',
  standalone: true,
  imports: [AppHeaderComponent, AppSidebarComponent, RouterLink, ReturnNavDirective, SeriesAccentDirective],
  templateUrl: './moto3-teams.page.html',
  styleUrls: [
...
export class Moto3TeamsPageComponent {
  ...
  private raw = signal<Moto3TeamStanding[]>([]);
  ...
  imgFailKey(card: Moto3TeamCard): string {
    return card.constructorId?.trim() || card.team;
  }

  initialsFor(card: Moto3TeamCard): string {
    return initialsForTeam(card.team);
  }

  cardHasDetail(card: Moto3TeamCard): boolean {
    return Boolean(card.constructorId?.trim());
  }

  teamLink(card: Moto3TeamCard): (string | number)[] {
    return this.seriesCtx.path('escuderias', card.constructorId);
  }

  teamLogoClass(card: Moto3TeamCard): string {
    return moto3TeamLogoGridClass(card.constructorId);
  }
