...
import type {
  Moto3TeamProfile,
  Moto3TeamProfileCareerRow,
  Moto3TeamProfileDriver,
} from './moto3.types';
import { moto3TeamBikeUrl, moto3TeamLogoCardClass, moto3TeamLogoUrl } from './moto3-media';
...
@Component({
  selector: 'app-moto3-team-profile-page',
  standalone: true,
  imports: [
    AppHeaderComponent,
    AppSidebarComponent,
  ...
  templateUrl: './moto3-team-profile.page.html',
  styleUrls: [
...
export class Moto3TeamProfilePageComponent {
  ...
  pilotProfileLink(driverId: string): (string | number)[] {
    return this.seriesCtx.path('pilotos', driverId);
  }
  ...
  logoUrl = computed(() => {
    ...
    return moto3TeamLogoUrl(p?.constructorId, p?.teamId, p?.logoUrl, p?.name);
  });

  profileLogoClass = computed(() => {
    const id = this.profile()?.constructorId;
    return `cp-logo-fallback ${moto3TeamLogoCardClass(id)}`;
  });

  bikeUrl = computed(() => moto3TeamBikeUrl(this.profile()?.bikeImageUrl));
  ...
  careerSeasonCount(p: Moto3TeamProfile): number {
    return p.careerHistoryPagination?.totalYears ?? p.careerHistory.length;
  }

  constructorChampion(h: Moto3TeamProfileCareerRow): boolean {
    return h.titleWon === true;
  }
  ...
  private applyProfileEmission(incoming: Moto3TeamProfile): void {
  ...
  private loadFullProfile(id: string): Observable<void> {
    ...
    return this.moto3.getTeamProfile(id, 1).pipe(
      tap((profile) => {
        if (!profile) return;
  ...
  private runProfileSideEffects(id: string, profile: Moto3TeamProfile): void {
  ...
  driverHeadshot(d: Moto3TeamProfileDriver): string {
  ...
  country2ForDriver(d: Moto3TeamProfileDriver): string {
