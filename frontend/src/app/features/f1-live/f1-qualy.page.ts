...
const SESSION_LABEL = 'QUALY';
const DURATION_MINUTES = 46; // Qualifying total window (Q1 18 + Q2 15 + Q3 13 = 46)

@Component({
  selector: 'app-f1-qualy-page',
  standalone: true,
  imports: [NgClass, RouterLink],
  templateUrl: './f1-qualy.page.html',
  styleUrl: './f1-fp1.page.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
})
export class F1QualyPageComponent implements OnInit, OnDestroy {
  ...
  elapsedDisplay = computed(() => {
    const sess = this.sessions().find(s => (s.sessionName || '').toLowerCase().includes('qualify') || (s.sessionName || '').toLowerCase().includes('qualif'))
                ?? this.sessions().find(s => s.sessionName === 'Qualifying');
    if (sess?.dateStart) {
      try {
