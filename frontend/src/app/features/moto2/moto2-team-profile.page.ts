...
import type {
  Moto2TeamProfile,
  Moto2TeamProfileCareerRow,
  Moto2TeamProfileDriver,
} from './moto2.types';
import { moto2TeamBikeUrl, moto2TeamLogoCardClass, moto2TeamLogoUrl } from './moto2-media';
...
@Component({
  selector: 'app-moto2-team-profile-page',
  standalone: true,
  imports: [
    AppHeaderComponent,
    AppSidebarComponent,
  ...
  templateUrl: './moto2-team-profile.page.html',
  styleUrls: [
...
export class Moto2TeamProfilePageComponent {
  ...
  profile = signal<Moto2TeamProfile | null>(null);
  ...
  logoUrl = computed(() => {
    ...
    return moto2TeamLogoUrl(p?.constructorId, p?.teamId, p?.logoUrl, p?.name);
  });

  profileLogoClass = computed(() => {
    const id = this.profile()?.constructorId;
    return `cp-logo-fallback ${moto2TeamLogoCardClass(id)}`;
  });

  bikeUrl = computed(() => moto2TeamBikeUrl(this.profile()?.bikeImageUrl));
  ...
  careerSeasonCount(p: Moto2TeamProfile): number {
    return p.careerHistoryPagination?.totalYears ?? p.careerHistory.length;
  }

  constructorChampion(h: Moto2TeamProfileCareerRow): boolean {
  ...
  private applyProfileEmission(incoming: Moto2TeamProfile): void {
  ...
  private loadFullProfile(id: string): Observable<void> {
    ...
    return this.moto2.getTeamProfile(id, 1).pipe(
      tap((profile) => {
        if (!profile) return;
  ...
  private runProfileSideEffects(id: string, profile: Moto2TeamProfile): void {
  ...
  driverHeadshot(d: Moto2TeamProfileDriver): string {
  ...
  country2ForDriver(d: Moto2TeamProfileDriver): string {
