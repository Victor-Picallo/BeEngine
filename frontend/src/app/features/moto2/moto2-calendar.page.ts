...
import { motoRaceHasCircuitMap } from '../motogp/moto-calendar-circuit.util';
import { MotoCircuitCardMapComponent } from '../motogp/moto-circuit-card-map.component';

type CalendarFilter = 'all' | 'completed' | 'upcoming';

interface Moto2CalendarCard {
...
@Component({
  selector: 'app-moto2-calendar-page',
  standalone: true,
  imports: [
    RouterLink,
    ReturnNavDirective,
  ...
  templateUrl: './moto2-calendar.page.html',
  styleUrl: '../calendar/f1-calendar.page.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class Moto2CalendarPageComponent {
  ...
  allCards = computed<Moto2CalendarCard[]>(() => {
    ...
    return this.calendar().map((race) => {
      ...
      const status: Moto2CalendarCard['status'] =
  ...
  filteredCards = computed<Moto2CalendarCard[]>(() => {
    const f = this.filter();
  ...
  raceCardLink(card: Moto2CalendarCard): (string | number)[] {
  ...
  private loadCompletedResults(calendar: JolpikaCalendarRace[]): void {
    ...
    forkJoin(
      completed.map((race) =>
        this.moto2.getRaceResults(race.round).pipe(
