...
export interface Moto2DriverCard {
  pos: number;
  driver: string;
  driverId: string;
  team: string;
...
function buildCards(rows: JolpikaDriverStanding[]): Moto2DriverCard[] {
  return rows.map((j) => {
...
export class Moto2PilotosPageComponent {
  ...
  cardHasProfile(card: Moto2DriverCard): boolean {
    return Boolean(card.driverId && card.driverId !== 'unknown');
  }

  profileLink(card: Moto2DriverCard): (string | number)[] {
  ...
  initialsFor(card: Moto2DriverCard): string {
